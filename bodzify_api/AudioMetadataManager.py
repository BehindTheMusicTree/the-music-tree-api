...
def get_specific_metadata_from_file(file, metadata_key: str):
    ...
    if fileExtensionLowered in [".wav", ".mp3"]:
        ...
        flacFileTags = _create_flac_object_dealing_with_eventual_temporary_file(
            file)
...
def get_metadata_dict_from_file(file, normalized_rating_max_value: Optional[int] = None):
    ...
    if fileExtensionLowered in [".wav", ".mp3"]:
        ...
        album_artists_name_string = _getalbum_artists_nametringTagFromId3FileTags(
            fileTags)
        ...
        fileTags = _create_flac_object_dealing_with_eventual_temporary_file(
            file)
        ...
        album_artists_name_string = _getalbum_artists_nametringTagFromFlacFileTags(
            fileTags)
        genre_name = _getgenre_name_tagFromFlacFileTags(fileTags)
        rating = _getEventuallyNormalizedRatingValueFromFlacFileTags(
            fileTags, normalized_rating_max_value)
...
def _getalbum_artists_nametringTagFromId3FileTags(id3FileTags: MutagenFile):
...
def _getEventuallyNormalizedRatingValueFromFlacFileTags(
    ...
    fileRating = _getFirstValueIntIfExistsOrNone(
        flacFileTags, VORBIS_TAG_KEYS.RATING)
    isRatingFromTraktor = False
    if fileRating is None:
        fileRating = _getFirstValueIntIfExistsOrNone(
            flacFileTags, VORBIS_TAG_KEYS.RATING_TRAKTOR)
...
def _getalbum_artists_nametringTagFromFlacFileTags(flacFileTags: FLAC):
    album_artists_name_stringRaw = (
...
def _getSpecificMetadataFromId3File(
    ...
    if metadataKey == METADATA_DICT_KEYS.TITLE:
        ...
        return _getalbum_artists_nametringTagFromId3FileTags(id3FileTags)
...
def _getSpecificMetadataFromFlacFile(
    ...
    if metadataKey == METADATA_DICT_KEYS.TITLE:
        ...
        return _getalbum_artists_nametringTagFromFlacFileTags(flacFileTags)
    elif metadataKey == METADATA_DICT_KEYS.GENRE_NAME:
