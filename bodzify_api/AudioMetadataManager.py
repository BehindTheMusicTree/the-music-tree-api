...
def get_metadata_dict_from_file(file, normalized_rating_max_value: Optional[int] = None):
    ...
    if file_extension_lowered in [".wav", ".mp3"]:
        ...
        title = _getTitleTag_from_id3_file_tags(file_tags)
        artist_name = _getartist_name_tag_from_id3_file_tags(file_tags)
        album_name = _get_album_name_tag_from_id3_file_tags(file_tags)
        album_artists_name_string = _get_album_artists_nametring_tag_from_id3_file_tags(
            file_tags)
        ...
        rating = _get_eventually_normalized_rating_value_from_id3_file_tags(
            file_tags, normalized_rating_max_value)
        language = _getlanguage_tag_from_id3_file_tags(file_tags)

    elif file_extension.lower() == ".flac":
        file_tags = _create_flac_object_dealing_with_eventual_temporary_file(
            file)
        title = _getTitleTagFromFlacFileTags(file_tags)
        artist_name = _getartist_name_tagFromFlacFileTags(file_tags)
        album_name = _get_album_name_tagFromFlacFileTags(file_tags)
        album_artists_name_string = _get_album_artists_nametring_tagFromFlacFileTags(
            file_tags)
        genre_name = _getgenre_name_tagFromFlacFileTags(file_tags)
        rating = _get_eventually_normalized_rating_value_from_flac_file_tags(
            file_tags, normalized_rating_max_value)
        language = _getlanguage_tagFromFlacFileTags(file_tags)
    else:
    ...
    metadata_dict[METADATA_DICT_KEYS.DURATION] = _get_duration_from_file_tags(
        file_tags=file_tags)
...
def get_specific_metadata_from_file(file, metadata_key: str):
    ...
    if file_extension_lowered in [".wav", ".mp3"]:
        file_tags = MutagenFile(file)
        return _getSpecificMetadataFromId3File(id3_file_tags=file_tags, metadataKey=metadata_key)
    elif file_extension_lowered == ".flac":
        flac_file_tags = _create_flac_object_dealing_with_eventual_temporary_file(
            file)
        return _getSpecificMetadataFromFlacFile(flac_file_tags=flac_file_tags, metadataKey=metadata_key)
    else:
        raise ValueError(FILE_EXTENSION_NOT_HANDLED_MESSAGE)
...
def _getTitleTag_from_id3_file_tags(id3_file_tags: MutagenFile):
    return _get_first_value_if_exists_or_none(id3_file_tags, ID3_TEXT_FRAMES.TITLE)


def _getartist_name_tag_from_id3_file_tags(id3_file_tags: MutagenFile):
...
def _get_album_artists_nametring_tag_from_id3_file_tags(id3_file_tags: MutagenFile):
...
def _get_eventually_normalized_rating_from_file_value(
        file_rating_value: int, normalized_rating_max_value: int, is_rating_from_traktor: bool = False):
    if file_rating_value is not None:
        if normalized_rating_max_value is not None:
            ...
            raise ValueError("Rating value not handled: " +
                             str(file_rating_value))
        else:
...
def _get_eventually_normalized_rating_value_from_id3_file_tags(
        id3_file_tags: MutagenFile, normalized_rating_max_value: int):
...
def _get_eventually_normalized_rating_value_from_flac_file_tags(
    ...
    if file_rating is None:
        file_rating = _get_first_value_int_if_exists_or_none(
            flac_file_tags, VORBIS_TAG_KEYS.RATING_TRAKTOR)
...
def _getgenre_name_tagFromFlacFileTags(flac_file_tags: FLAC):
...
def _getlanguage_tagFromFlacFileTags(flacFile: FLAC):
    return _get_first_value_if_exists_or_none(flacFile, VORBIS_TAG_KEYS.LANGUAGE)


def _getSpecificMetadataFromId3File(
        id3_file_tags: MutagenFile, metadataKey: str, normalized_rating_max_value: int = None):
    if metadataKey == METADATA_DICT_KEYS.TITLE:
        return _getTitleTag_from_id3_file_tags(id3_file_tags)
    elif metadataKey == METADATA_DICT_KEYS.ARTIST_NAME:
        return _getartist_name_tag_from_id3_file_tags(id3_file_tags)
        ...
        return _get_album_artists_nametring_tag_from_id3_file_tags(id3_file_tags)
        ...
        return _getlanguage_tag_from_id3_file_tags(id3_file_tags)


def _getSpecificMetadataFromFlacFile(
        flac_file_tags: FLAC, metadataKey: str, normalized_rating_max_value: int = None):
    if metadataKey == METADATA_DICT_KEYS.TITLE:
        return _getTitleTagFromFlacFileTags(flac_file_tags)
    elif metadataKey == METADATA_DICT_KEYS.ARTIST_NAME:
        return _getartist_name_tagFromFlacFileTags(flac_file_tags)
    elif metadataKey == METADATA_DICT_KEYS.ALBUM_NAME:
        return _get_album_name_tagFromFlacFileTags(flac_file_tags)
    elif metadataKey == METADATA_DICT_KEYS.ALBUM_ARTISTS_NAMES:
        return _get_album_artists_nametring_tagFromFlacFileTags(flac_file_tags)
    elif metadataKey == METADATA_DICT_KEYS.GENRE_NAME:
        return _getgenre_name_tagFromFlacFileTags(flac_file_tags)
    elif metadataKey == METADATA_DICT_KEYS.DURATION:
        ...
        return _getlanguage_tagFromFlacFileTags(flac_file_tags)
...
def _get_id3_file_tags_updated_with_metadata_value(
    ...
    if update_metadata_key == METADATA_DICT_KEYS.TITLE:
        ...
        if normalizedRating is not None:
            ...
            id3_file_tags.add(
                POPM(email=ID3_RATING_APP_EMAIL, rating=id3Rating))
    ...
    id3_file_tags.add(textFrameClass(
        encoding=3, text=update_metadata_dict[update_metadata_key]))

    return id3_file_tags


def _getFileRatingFromNormalizedValue(
        normalizedRating: int, normalized_rating_max_value: int, ratingFileProfile: str):
    starRatingBase10 = (int)((normalizedRating * 10)/normalized_rating_max_value)
...
def _getFlacFileTagsUpdatedIfValueSpecified(
    ...
    if metadata_dictKey in metadata_update_dict:
        ...
        if value is not None:
            if vorbisTagKey not in flac_file_tags:
                flac_file_tags[vorbisTagKey] = [1]
            flac_file_tags[vorbisTagKey] = metadata_update_dict[metadata_dictKey]
        elif vorbisTagKey in flac_file_tags:
            del flac_file_tags[vorbisTagKey]
