...
class TestCase(GenreTestCase):
    ...
    def test_parent_none_then_root_itself(self):
        ...
        response = self._put_genre(uuid=punk_genre.uuid, data_dict={InputFields.PARENT: None})
    ...
    def test_one_acendant_then_root_is_parent(self):
        ...
        response = self._post_genre(kwargs={InputFields.NAME: "Punk", InputFields.PARENT: rock.uuid})
    ...
    def test_two_acendant_then_root_is_parent_of_parent(self):
        rock_genre = self.model_fixture_factory.create_genre(name="Rock")
        punk_genre = self.model_fixture_factory.create_genre(name="Punk", parent=rock_genre)
        response = self._post_genre(kwargs={InputFields.NAME: "Punk hardcore", InputFields.PARENT: punk_genre.uuid})
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_three_acendants_then_root_is_parent_of_parent_of_parent(self):
        ...
        data = {InputFields.NAME: "Punk hardcore japonais", InputFields.PARENT: hardcorepunk_genre.uuid}
        response = self._post_genre(kwargs=data)
        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_genre.root == rock_genre
