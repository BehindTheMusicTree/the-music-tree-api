...
class TestCase(GenreTestCase):
    ...
    def test_longest(self):
        ...
        response = self._post_genre(kwargs={InputFields.NAME: genre_name})
        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_genre.name == genre_name

    def test_error_too_long(self):
        response = self._post_genre(kwargs={InputFields.NAME: "a" * (settings.CRITERIA_NAME_LEN_MAX + 1)})
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_multiple_values_then_error(self):
        response = self._post_genre(kwargs={InputFields.NAME: ["value", "value2"]})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
