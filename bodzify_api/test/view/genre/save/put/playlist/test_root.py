...
class TestCase(GenreTestCase):
    ...
    def test_new_root_then_update_root_of_descendants(self):
        ...
        updated_punk_genre_playlist = CriteriaPlaylist.objects.get(user=self.test_user1, criteria=punk_genre)
        assert updated_punk_genre_playlist.root == root_playlist

        updated_punkhardcore_genre_playlist = CriteriaPlaylist.objects.get(
            user=self.test_user1, criteria=punkhardcore_genre)
        assert updated_punkhardcore_genre_playlist.root == root_playlist

        updated_frenchhardcore_genre_playlist = CriteriaPlaylist.objects.get(
