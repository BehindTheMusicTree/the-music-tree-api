...
class TestCase(GenreTestCase):
    ...
    def test_error_when_parent_is_one_of_descendants(self):
        ...
        response = self._put_genre(uuid=rock_genre.uuid, data_dict={PutFields.PARENT: punkhardcore_genre.uuid})

        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_error_when_parent_is_itself(self):
        rock_genre = self.model_fixture_factory.create_genre(name="Rock")
        response = self._put_genre(uuid=rock_genre.uuid, data_dict={PutFields.PARENT: rock_genre.uuid})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
