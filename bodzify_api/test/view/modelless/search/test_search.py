...
class TestCase(ApiViewTestCase):
    ...
    def test_query_in_track_artist_and_album(self):
        ...
        assert results[Artist.__name__][0][ARTIST_FIELDS.NAME] == sum41_artist.name  # type: ignore
        assert results[Album.__name__][0][ALBUM_FIELDS.NAME] == jailesum_album.name  # type: ignore
    ...
    def test_the_all_string_including_a_track(self):
        ...
        assert results[LibraryTrack.__name__][0][track_title_key] == werealltoblame_track.title  # type: ignore
        simple_playlist_name_key = SIMPLE_PLAYLIST_FIELDS.NAME
        assert results[SimplePlaylist.__name__][0][simple_playlist_name_key] == SIMPLE_PLAYLIST_SPECIAL_NAMES.ALL
    ...
    def test_non_sensitiveness(self):
        ...
        assert response_json[ApiViewTestCase.RESPONSE_FIELDS.OVERALL_TOTAL] == 2
        results = response_json[ApiViewTestCase.RESPONSE_FIELDS.RESULTS]
        assert results[CriteriaPlaylist.__name__][0][CRITERIA_PLAYLIST_FIELDS.NAME] == rap_criteria_name
        assert results[CriteriaPlaylist.__name__][1][CRITERIA_PLAYLIST_FIELDS.NAME] == us_rap_criteria_name
