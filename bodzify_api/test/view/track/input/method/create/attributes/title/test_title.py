...
class TestCase(TrackTestCase):
    ...
    def test_not_povided_then_set_from_filename_without_dots(self):
        ...
        assert self.lib_track_saved.title == self.LibTrackGenericSamplesFilenameWithoutExtension.TAGS_NONE
    ...
    def test_not_povided_then_set_from_filename_with_dots(self):
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert self.lib_track_saved.title == filename_without_extension
    ...
    def test_not_povided_then_set_from_filename_with_spaces_removing_extra_spaces(self):
        ...
        response = self.post_lib_track_with_specific_sample(filename_with_extension)
        assert response.status_code == status.HTTP_201_CREATED
        assert self.lib_track_saved.title == title_expected
