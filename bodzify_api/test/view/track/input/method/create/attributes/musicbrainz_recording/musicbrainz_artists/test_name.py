...
class TestCase(TrackTestCase):
    ...
    def test_one_then_ok(self):
        ...
        assert self.lib_track_saved.musicbrainz_recording.musicbrainz_artists.all()[0].name == "Queen"  # type: ignore

    def test_multiple_then_ok(self):
        response = self.post_lib_track_with_specific_sample("oostil_Juan Hansen.mp3")
        assert response.status_code == status.HTTP_201_CREATED
        artists_names = [
            artist.name for artist in self.lib_track_saved.musicbrainz_recording.musicbrainz_artists.all()  # type: ignore
        ]
