...
class TestCase(LibTrackTestCase):
    ...
    def test_no_genre_then_in_the_all_lib_track_mixin_and_genreless_playlists(self):
        response = self._post_lib_track_with_generic_sample_no_tags()

        assert response.status_code == status.HTTP_201_CREATED

        assert self.test_user1.all_lib_track_mixin.library_tracks.filter(uuid=self.saved_lib_track.uuid).exists()
