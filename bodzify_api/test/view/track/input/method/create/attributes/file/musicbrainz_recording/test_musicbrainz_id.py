...
class TestCase(LibTrackTestCase):
    ...
    def test_totaleclipe_5m35_flac_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.RECORDING_TOTAL_ECLIPSE_5M35_FLAC)
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_different_format_but_same_musicbrainz_recording(self):
        response = self._post_lib_track(TestLibTrackFilename.RECORDING_JUAN_HANSEN_OOSTIL_DROWN_MASSANO_REMIX_7M20_FLAC)
        assert response.status_code == status.HTTP_201_CREATED
