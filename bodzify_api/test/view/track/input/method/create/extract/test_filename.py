...
class FilenameTestCase(ApiViewTestCase):
    ...
    def test_title_and_artist_name_in_data_then_filename_with_artist_and_title(self):
        ...
        assert self.saved_lib_track.filename == f"Roméo_-_ImHere.{ApiViewTestCase.SAMPLE_MINE_TRACK_EXTENSION}"
    ...
    def test_title_and_artist_with_spaces_then_filename_without_spaces(self):
        ...
        response = self.extract_default_mine_track(data_json=data)
        assert response.status_code == status.HTTP_201_CREATED  # type: ignore
        assert self.saved_lib_track.filename == f"Rom_éo_-_Im_Here.{ApiViewTestCase.SAMPLE_MINE_TRACK_EXTENSION}"
    ...
    def test_title_and_artist_with_special_characyers_then_filename_without_them(self):
        ...
        response = self.extract_default_mine_track(data_json=data)
        assert response.status_code == status.HTTP_201_CREATED  # type: ignore
        assert self.saved_lib_track.filename == f"Roméo_-_Im_Here.{ApiViewTestCase.SAMPLE_MINE_TRACK_EXTENSION}"
    ...
    def test_only_title_in_data_then_filename_with_title(self):
        ...
        response = self.extract_default_mine_track(data_json=data)
        assert response.status_code == status.HTTP_201_CREATED  # type: ignore
        assert self.saved_lib_track.filename == f"{title}.{ApiViewTestCase.SAMPLE_MINE_TRACK_EXTENSION}"
