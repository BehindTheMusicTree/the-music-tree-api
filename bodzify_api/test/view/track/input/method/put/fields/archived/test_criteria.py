...
class TestCase(LibTrackTestCase):
    ...
    def test_archived_lib_track_then_criteria_has_plus_1_archived_lib_tracks(self):
        ...
        self.model_fixture_factory.create_lib_track_with_file(title="not archived 2", genre=criteria)
        self.model_fixture_factory.create_lib_track_with_file(title="not archived 3", genre=criteria)
        self.model_fixture_factory.create_lib_track_with_file(title="archived 1", genre=criteria, archived=True)
        self.model_fixture_factory.create_lib_track_with_file(title="archived 2", genre=criteria, archived=True)
        self.model_fixture_factory.create_lib_track_with_file(title="archived 3", genre=criteria, archived=True)
        track_love = self.model_fixture_factory.create_lib_track_with_file(title="Love", genre=criteria)
        data = {PutFields.ARCHIVED: "true"}
        response = self._put_lib_track(uuid=track_love.uuid, data_dict=data)
    ...
    def test_unarchived_then_criteria_has_minus_1_archived_lib_tracks(self):
        ...
        self.model_fixture_factory.create_lib_track_with_file(title="archived 1", genre=criteria, archived=True)
        self.model_fixture_factory.create_lib_track_with_file(title="archived 2", genre=criteria, archived=True)
        track = self.model_fixture_factory.create_lib_track_with_file(title="Love", genre=criteria, archived=True)
