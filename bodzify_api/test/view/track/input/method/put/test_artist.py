...
class TestCase(NullableFieldTestCase):
    ...
    def test_not_provided_then_unchanged(self):
        ...
        lib_track = G(LibraryTrack, user=self.test_user, title="Love", artist=artist)
    ...
    def test_empty_then_none(self):
        ...
        lib_track = G(LibraryTrack, user=self.test_user, title="koko", artist=artist_old)
    ...
    def test_not_none_then_update(self):
        ...
        data = {PUT_FIELDS.ARTIST_NAME: artist_new.name}  # type: ignore
        response = self.put_lib_track(lib_track_uuid=lib_track.uuid, data_dict=data)  # type: ignore
        assert response.status_code == status.HTTP_200_OK  # type: ignore
    ...
    def test_delete_old_one_because_nothing_linked_to_it(self):
        ...
        track = G(LibraryTrack, user=self.test_user, title="Foire", artist=artist)
    ...
    def test_not_delete_old_one_because_a_track_linked_to_it(self):
        ...
        artist = G(Artist, user=self.test_user, name=artist_name)
        track = G(LibraryTrack, user=self.test_user, title="Foire", artist=artist)
        G(LibraryTrack, user=self.test_user, title="Josie", artist=artist)
