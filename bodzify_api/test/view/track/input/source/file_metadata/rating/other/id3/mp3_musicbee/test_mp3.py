...
from bodzify_api.test.view.track.input.source.file_metadata.rating.LibraryTrackFromFileMetadataRatingWithHalfAttributeTestCase \
    import LibraryTrackFromFileMetadataRatingWithHalfAttributeTestCase


@pytest.mark.django_db
class TestCase(LibraryTrackFromFileMetadataRatingWithHalfAttributeTestCase):

    def test_0_then_0(self):
        response = self.post_lib_track_with_specific_sample("0 star.mp3")
        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_lib_track.rating == 0

    def test_0_and_half_then_1(self):
        response = self.post_lib_track_with_specific_sample("0 5 star.mp3")
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_1_and_half_then_3(self):
        response = self.post_lib_track_with_specific_sample("1 5 stars.mp3")
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_2_and_half_then_5(self):
        response = self.post_lib_track_with_specific_sample("2 5 stars.mp3")
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_3_and_half_then_7(self):
        response = self.post_lib_track_with_specific_sample("3 5 stars.mp3")
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_4_and_half_then_9(self):
        response = self.post_lib_track_with_specific_sample("4 5 stars.mp3")
        assert response.status_code == status.HTTP_201_CREATED
