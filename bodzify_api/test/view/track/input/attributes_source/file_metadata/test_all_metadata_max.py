...
class TextCase(LibTrackTestCase):
    ...
    def test_id3v1_short_flac_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V1_SMALL_FLAC)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v1_long_flac_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V1_BIG_FLAC)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v1_short_wav_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V1_SMALL_WAV)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v1_long_wav_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V1_BIG_WAV)
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_id3v2_long_mp3_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V2_BIG_MP3)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v2_short_flac_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V2_SMALL_FLAC)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v2_long_flac_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V2_BIG_FLAC)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v2_short_wav_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V2_SMALL_WAV)
        assert response.status_code == status.HTTP_201_CREATED

    def test_id3v2_long_wav_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_ID3V2_BIG_WAV)
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_riff_long_wav_then_ok(self):
        response = self._post_lib_track(TestLibTrackFilename.METADATA_LONG_A_RIFF_BIG_WAV)
        assert response.status_code == status.HTTP_201_CREATED
