...
class TestCase(LibTrackTestCase):
    ...
    def test_new_genre_then_in_new_genre_playlist_and_all_playlist(self):
        ...
        manual_playlists = ManualPlaylist.objects.filter(user=self.test_user1, base_playlist__in=track_playlists)
        assert manual_playlists.filter(name=LibTrackMixinSpecialNames.ALL).exists()
    ...
    def test_existing_genre_then_track_in_existing_playlist_and_all_playlist(self):
        ...
        playlist_all = ManualPlaylist.objects.get(name=LibTrackMixinSpecialNames.ALL).base_playlist
    ...
    def test_existing_genre_with_2_successive_ascendants_then_track_in_3_existing_playlists(self):
        ...
        lib_track_manual_playlists = ManualPlaylist.objects.filter(
            user=self.test_user1, base_playlist__in=lib_track_playlists)
        assert lib_track_manual_playlists.filter(name=LibTrackMixinSpecialNames.ALL).exists()
