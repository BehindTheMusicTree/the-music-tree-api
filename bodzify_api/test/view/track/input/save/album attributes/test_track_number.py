...
class TestCase(LibTrackTestCase, NullablePositiveIntBodyDataTestCase):
    ...
    def test_empty_then_none(self):
        response = self._post_uploaded_track(LibTrackTestFilename.METADATA_NONE_MP3, **{PostFields.TRACK_NUMBER: None})

        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_object.track_number == None

    def test_empty_string_then_none(self):
        response = self._post_uploaded_track(LibTrackTestFilename.METADATA_NONE_MP3, **{PostFields.TRACK_NUMBER: ''})

        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_object.track_number == None

    def test_string_castable_then_ok(self):
        response = self._post_uploaded_track(LibTrackTestFilename.METADATA_NONE_MP3, **{PostFields.TRACK_NUMBER: '5'})
    ...
    def test_zero_then_400_bad_request(self):
        response = self._post_uploaded_track(LibTrackTestFilename.METADATA_NONE_MP3, **{PostFields.TRACK_NUMBER: 0})
    ...
    def test_negative_then_400_bad_request(self):
        response = self._post_uploaded_track(LibTrackTestFilename.METADATA_NONE_MP3, track_number=-1)
    ...
    def test_float_then_400_bad_request(self):
        response = self._post_uploaded_track(LibTrackTestFilename.METADATA_NONE_MP3, **{PostFields.TRACK_NUMBER: 5.5})
