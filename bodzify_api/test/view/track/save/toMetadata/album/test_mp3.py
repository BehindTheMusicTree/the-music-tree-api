...
class TestCase(ApiViewTestCase):
    ...
    def test_longest(self):
        ...
        data = {
            SCHEMA_TRACK_ATTRIBUTES_LABEL.ALBUM_NAME: album_name
        }
        response = self.post_sample_track(sample_filename="sample.mp3", data_json=data)
        assert response.status_code == status.HTTP_201_CREATED
        key = AudioMetadataManager.METADATA_DICT_KEYS.ALBUM_NAME
        assert self.saved_track_metadata[key] == album_name
