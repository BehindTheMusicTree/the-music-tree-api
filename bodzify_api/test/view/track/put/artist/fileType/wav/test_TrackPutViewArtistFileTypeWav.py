...
class TrackViewPutArtistFileTypeWavTestCase(TrackViewTestCase):

    fixtures = ['initial_data', 'TestUserData', 'TestViewTrackPutArtistFileTypeWavData']
    sampleDirectoryRelativePath = "test/view/track/put/artist/fileType/wav/sample/"


    """
    null artist. There shouldn't be a artist tag in the file.
    """
    def test_trackPutArtistFileTypeWavNone(self):
        data = {
            "artistName": None,
        }
    ...
    """
    def test_trackPutArtistFileTypeWavLongestString(self):
        data = {
            "artistName": "a" * 100,
        }
        response = self._loginAndPutSampleTrack(trackUuid="36nS4LVDssLh4BvTAKKKKO", data=data)
