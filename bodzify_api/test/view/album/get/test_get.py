...
class TestCase(AlbumViewTestCase):
    ...
    def test_get(self):
        ...
        self.model_fixture_factory.create_lib_track(title="In Too Deep", artists=[sum41_artist], album=chuck_album)
        self.model_fixture_factory.create_lib_track(title="We're All To Blame",
                                                    artists=[sum41_artist],
                                                    album=chuck_album)
        self.model_fixture_factory.create_lib_track(title="Pieces", artists=[sum41_artist], album=allkiller_album)
        response = self._get()
