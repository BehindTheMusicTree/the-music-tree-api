...
class TestCase(AlbumTestCase):
    ...
    def test_all_uploaded_tracks_with_positions_then_order_by_position_asc(self):
        ...
        assert result_tracks[0][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_1st_position.title
        assert result_tracks[1][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_12th_position.title
        assert result_tracks[2][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_45th_position.title
    ...
    def test_some_uploaded_tracks_with_positions_then_order_by_position_asc_then_those_with_no_position_by_title_asc(
        ...
        assert response.status_code == status.HTTP_200_OK
        result_tracks = self.result[to_camel_case(RetrieveFields.LIB_TRACKS_NOT_ARCHIVED_SORTED_PUBLIC)]
        assert result_tracks[0][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_4th_position.title
        assert result_tracks[1][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_112th_position.title
        assert result_tracks[2][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_no_position_1.title
        assert result_tracks[3][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_no_position_2.title
        assert result_tracks[4][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_no_position_3.title
        assert result_tracks[5][to_camel_case(LibTrackOutputFields.TITLE)] == uploaded_track_no_position_4.title

    def test_duration(self):
        album = self.model_fixture_factory.create_album(name="Chuck")
        self.model_fixture_factory.create_uploaded_track_with_file(
            title='ciline', album=album, test_uploaded_track_filename=LibTrackTestFilename.DURATION_277S_MP3)
        self.model_fixture_factory.create_uploaded_track_with_file(
            title='tokyo', album=album, test_uploaded_track_filename=LibTrackTestFilename.DURATION_472S_WAV)
