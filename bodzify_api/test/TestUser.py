...
class TestUser():
    lib_abs_path: Path
    LIBRARIES_PATH_relative_to_media_dir: Path
    django_user: User
    ...
    def __set_up_dirs(self):
        self.lib_abs_path = settings.LIBRARIES_PATH / Path(settings.USER_LIBRARIES_DIR_NAME_PREFIXE +
                                                           str(self.django_user.pk))
        if not self.lib_abs_path.exists():
            os.makedirs(self.lib_abs_path)

        self.LIBRARIES_PATH_relative_to_media_dir = \
            Path(settings.LIBRARIES_DIR_NAME) / (settings.USER_LIBRARIES_DIR_NAME_PREFIXE + str(self.django_user.pk))
        self.lib_abs_path = settings.MEDIA_ROOT / self.LIBRARIES_PATH_relative_to_media_dir
