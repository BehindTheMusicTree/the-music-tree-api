...
def _convert_metadata_format(format: MetadataFormat) -> AudiometaMetadataFormat:
    """Convert local MetadataFormat to audiometa MetadataFormat."""
    format_map = {
        MetadataFormat.ID3V1: AudiometaMetadataFormat.ID3V1,
        MetadataFormat.ID3V2: AudiometaMetadataFormat.ID3V2,
        MetadataFormat.VORBIS: AudiometaMetadataFormat.VORBIS,
        MetadataFormat.RIFF: AudiometaMetadataFormat.RIFF,
    }
...
def get_single_format_app_metadata(
    ...
    audiometa_format = _convert_metadata_format(tag_format)
    unified_metadata = audiometa.get_unified_metadata(
        file=file_path,
        normalized_rating_max_value=normalized_rating_max_value,
        metadata_format=audiometa_format,
    )
...
def delete_metadata(file: FILE_TYPE, tag_format: MetadataFormat | None = None) -> bool:
    ...
    file_path = _get_file_path(file)
    audiometa_format = _convert_metadata_format(tag_format) if tag_format else None
    return audiometa.delete_all_metadata(file=file_path, metadata_format=audiometa_format)
