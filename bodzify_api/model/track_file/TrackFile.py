...
class TrackFile(models.Model):
    ...
    size_in_ko = models.GeneratedField(expression=F(AttributesLabels.SIZE_IN_BYTES) / 1024,  # type: ignore
                                       output_field=models.DecimalField(max_digits=8, decimal_places=2),
                                       db_persist=True)
    size_in_mo = models.GeneratedField(expression=F(AttributesLabels.SIZE_IN_BYTES) / (1024 * 1024),  # type: ignore
    ...
    def save(self, *args, **kwargs):
        ...
        super().save(update_fields=[AttributesLabels.BITRATE_IN_KBPS])
            ...
            if not audio_metadata.is_flac_file_md5_valid(self.file.path):
                try:
                    ...
                    raise ValidationError(
                        {AttributesLabels.FILE: ["The Flac file md5 check failed and could not be corrected. The " +
                                                 "file is probably corrupted."]}
                    )
            else:
                self.flac_md5_has_been_corrected = False
            super().save(update_fields=[AttributesLabels.FLAC_MD5_HAS_BEEN_CORRECTED])
