...
-from bodzify_api.utils import audio_fingerprinter
+from bodzify_api.model.track.file.TrackFileManager import TrackFileManager
...
+                          max_length= settings.FILE_PATH_MAX_LENGTH)
     ...
     # The duration is None between the time the file is saved and the time the metadata are analyzed.
...
-
-        duration_in_sec= audio_metadata.get_specific_metadata_from_file(
- file=temp_file_path,
-            normalized_metadata_key=NormalizedMetadataKeys.DURATION_IN_SEC)
-        self.duration_in_sec= int(duration_in_sec)  # type: ignore
-
...
-                                                                                  track_file=temp_file_path,
-                                                                                  title=library_track.title,)
-
- if fingerprinting_result.is_success:
-                # For some reasons, the duration_in_sec returned by the fingerprinting service is sometimes wrong.
-                # Until a further investigation, we use the duration_in_sec from the metadata.
-                # self.duration_in_sec = fingerprinting_result.duration_in_sec
-
...
- {Fields.FILE: [f"The file '{self.filename}' has the same fingerprint as "
...
- if self.fingerprint_missing_cause.code == FingerprintMissingCauseCode.Codes.AUDIO_META_AMALYSIS_DISABLED:
...
-)
+        manager= TrackFileManager(self)
+ manager.update_file_tags(normalized_metadata)
...
+        fingerprinting_result= manager.manage_fingerprint()
+ manager.manage_musicbrainz_recording(fingerprinting_result)
...
+        self.bitrate_in_kbps= manager.get_bitrate()
         super().save(update_fields=[Fields.BITRATE_IN_KBPS])
...
- except Exception:
- raise ValidationError(
- {Fields.FILE: ["The Flac file md5 check failed and could not be corrected. The " +
-                                       "file is probably corrupted."]}
-)
- else:
-                self.flac_md5_has_been_corrected= False
+ if manager.handle_flac_md5():
             super().save(update_fields=[Fields.FLAC_MD5_HAS_BEEN_CORRECTED])
