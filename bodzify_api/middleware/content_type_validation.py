...
class ContentTypeValidationMiddleware:
    """Middleware to enforce proper Content-Type headers and reject double-encoded JSON."""

    def __init__(self, get_response):
        self.get_response = get_response

    def __call__(self, request: Union[HttpRequest, Request]) -> HttpRequest:
        if request.method in ['POST', 'PUT', 'PATCH']:
            ...
            if content_type not in ['application/json', 'multipart/form-data']:
                raise UnsupportedMediaType(
                    'Unsupported content type. Use application/json for regular requests or multipart/form-data for file uploads'
                )
