...
class AppValidationError(ValidationError):
    ...
    def __init__(self, field: str, message: str, code: FieldValidationErrorCode, is_field_validation: bool = False):
        self.field = field
        self.error_detail = {
            'message': message,
            'code': code.value
        ...
        self.is_field_validation = is_field_validation

        # Create the error structure based on context
        error_dict = self.error_detail if is_field_validation else {field: self.error_detail}
    ...
    def from_field(cls, field: str, message: str, code: FieldValidationErrorCode) -> 'AppValidationError':
        ...
        """
        return cls(field, message, code, is_field_validation=True)
    ...
    def from_serializer(cls, field: str, message: str, code: FieldValidationErrorCode) -> 'AppValidationError':
        ...
        """
        return cls(field, message, code, is_field_validation=False)
    ...
    def is_field_level_validation(self) -> bool:
        """Check if this is a field-level validation error."""
        return self.is_field_validation
