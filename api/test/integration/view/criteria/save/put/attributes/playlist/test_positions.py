...
class TestCase(GenreTestCase):
    ...
    def test_new_parent_then_tracks_in_same_order_and_added_at_the_beginning(self) -> None:
        ...
        track_rock_added_second = self.model_fixture_factory.create_track_with_file(
        ...
        track_punk_added_third = self.model_fixture_factory.create_track_with_file(
            title="punk song", genre=genre_punk, use_manager_for_genre_playlist_adding=True)
        track_punk_added_fourth = self.model_fixture_factory.create_track_with_file(
        ...
        assert rock_playlist_tracks_not_archived_dict_by_position[1] == track_punk_added_fourth
        assert rock_playlist_tracks_not_archived_dict_by_position[2] == track_punk_added_third
        assert rock_playlist_tracks_not_archived_dict_by_position[3] == track_rock_added_second
    ...
    def test_new_parent_then_tracks_in_same_order_and_added_at_the_beginning_of_parent_of_parent(self) -> None:
        ...
        track_previously_second_in_guitare = self.model_fixture_factory.create_track_with_file(
            title="guitare2", genre=genre_guitare, use_manager_for_genre_playlist_adding=True)
        track_previously_first_in_guitare = self.model_fixture_factory.create_track_with_file(
        ...
        assert genre_guitare.criteria_playlist.track_playlist_rels.get(
            track=track_previously_first_in_guitare).position == 1
        assert genre_guitare.criteria_playlist.track_playlist_rels.get(
            track=track_previously_second_in_guitare).position == 2

        genre_rock = self.model_fixture_factory.create_genre(name="Rock", parent=genre_guitare)

        track_previously_second_in_rock = self.model_fixture_factory.create_track_with_file(
            title="rock2",
            genre=genre_rock,
            use_manager_for_genre_playlist_adding=True)
        track_previously_first_in_rock = self.model_fixture_factory.create_track_with_file(
            title="rock1",
            genre=genre_rock,
            use_manager_for_genre_playlist_adding=True)

        genre_punk = self.model_fixture_factory.create_genre(name="Punk")
        track_previously_second_in_punk = self.model_fixture_factory.create_track_with_file(
            title="punk2",
            genre=genre_punk,
            use_manager_for_genre_playlist_adding=True)
        track_previously_first_in_punk = self.model_fixture_factory.create_track_with_file(
            title="punk1",
            genre=genre_punk,
            use_manager_for_genre_playlist_adding=True)
        ...
        tracks_uuids_positions = {
            relation.track.uuid: relation.position for relation in track_playlist_rels}
        assert tracks_uuids_positions[track_previously_first_in_punk.uuid] == 1
        assert tracks_uuids_positions[track_previously_second_in_punk.uuid] == 2
        assert tracks_uuids_positions[track_previously_first_in_rock.uuid] == 3
        assert tracks_uuids_positions[track_previously_second_in_rock.uuid] == 4
        assert tracks_uuids_positions[track_previously_first_in_guitare.uuid] == 5
        assert tracks_uuids_positions[track_previously_second_in_guitare.uuid] == 6
    ...
    def test_new_parent_not_acendant_of_old_parent_then_update_positions_in_old_parent(self) -> None:
        ...
        track_rock_added_third = self.model_fixture_factory.create_track_with_file(
            title="Rock song 1", genre=genre_rock, use_manager_for_genre_playlist_adding=True)
        track_punk_added_forth = self.model_fixture_factory.create_track_with_file(
    ...
    def test_new_parent_undirect_ascendant_of_old_parent_then_update_positions_in_criterias_in_between(self) -> None:
        ...
        track_second_in_punk = self.model_fixture_factory.create_track_with_file(
            title="Punk song", genre=genre_punk, use_manager_for_genre_playlist_adding=True)
        ...
        assert response.status_code == status.HTTP_200_OK
        track_playlist_rel: TrackPlaylistRel = \
            TrackPlaylistRel.objects.get(user=self.test_user1,
                                         playlist=genre_punk.criteria_playlist,
                                         track=track_second_in_punk)
