...
class TestCase(UserTestCase):
    ...
    def test_delete_then_lib_dir_removed(self):
        ...
        self.model_fixture_factory.create_track_with_file(user=user, title='Dr mo')
    ...
    def test_delete_then_track_removed(self):
        user = self.model_fixture_factory.create_user()
        self._login_as_user(user)
        title = 'Dr mo'
        self.model_fixture_factory.create_track_with_file(user=user, title=title)
        assert Track.objects.filter(user=user, title=title).count() == 1

        self._login_as_test_admin()
        response = self._delete_user(user.pk)

        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert Track.objects.filter(user=user, title=title).count() == 0
    ...
    def test_delete_then_album_removed(self):
        ...
        self.model_fixture_factory.create_track_with_file(user=user, title='Skyfall', album=album)
    ...
    def test_delete_then_artist_removed(self):
        ...
        self.model_fixture_factory.create_track_with_file(user=user, title='Skyfall', artists=[artist])
