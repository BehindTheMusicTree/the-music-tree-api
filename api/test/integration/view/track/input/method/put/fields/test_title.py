...
class TestCase(TrackTestCase, PutBodyDataTestCase):
    ...
    def test_not_empty_then_ok(self):
        ...
        title_new = "a"
        response = self._put_track(track.uuid, **{PutFields.TITLE: title_new})
    ...
    def test_not_provided_then_unchanged(self):
        old_title = "Love"
        track = self.model_fixture_factory.create_track_with_file(title=old_title)
    ...
    def test_empty_then_400_bad_request(self):
        ...
        response = self._put_track(track.uuid, **{PutFields.TITLE: ""}
                                   )
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_provided_then_update(self):
        title = "a"
        track = self.model_fixture_factory.create_track_with_file(title=title)

        response = self._put_track(track.uuid, **{PutFields.TITLE: title})
