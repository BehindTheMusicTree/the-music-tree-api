...
class TestCase(TrackTestCase):
    ...
    def test_archived_track_then_manual_playlist_has_plus_1_archived_tracks(self):
        manual_playlist = self.model_fixture_factory.create_manual_playlist(name='teuf')

        track = self.model_fixture_factory.create_track_with_file(title="not archived 1")
        self.model_fixture_factory.create_track_playlist_rel(playlist=manual_playlist, track=track)

        track_archived = self.model_fixture_factory.create_track_with_file(title="archived 1", archived=True)
        self.model_fixture_factory.create_track_playlist_rel(
            playlist=manual_playlist, track=track_archived)

        response = self._put_track(uuid=track.uuid, **{PutFields.ARCHIVED: "true"})
    ...
    def test_archived_track_then_criteria_playlist_has_plus_1_archived_tracks(self):
    ...
    def test_archived_track_then_decrement_next_positions(self):
        manual_playlist = self.model_fixture_factory.create_manual_playlist(name="teuf")
        track1 = self.model_fixture_factory.create_track_with_file(title="not archived 1")
        self.model_fixture_factory.create_track_playlist_rel(
            playlist=manual_playlist.playlist, track=track1)
        track_to_archive = self.model_fixture_factory.create_track_with_file(title="to archived 2")
        self.model_fixture_factory.create_track_playlist_rel(
            playlist=manual_playlist.playlist, track=track_to_archive)
        track3 = self.model_fixture_factory.create_track_with_file(title="not archived 3")
        self.model_fixture_factory.create_track_playlist_rel(
            playlist=manual_playlist.playlist, track=track3)
        track4 = self.model_fixture_factory.create_track_with_file(title="not archived 4")
        self.model_fixture_factory.create_track_playlist_rel(
            playlist=manual_playlist.playlist, track=track4)
        ...
        response = self._put_track(uuid=track_to_archive.uuid, **{PutFields.ARCHIVED: "true"})
        ...
        assert cast(TrackPlaylistRel, archived_track_playlist_rels.first()
                    ).track == track_to_archive

    def test_unarchived_track_then_in_first_position_of_playlist(self):
        manual_playlist = self.model_fixture_factory.create_manual_playlist(name="Cuisine")
        track1 = self.model_fixture_factory.create_track_with_file(title="not archived 1")
        self.model_fixture_factory.create_track_playlist_rel(playlist=manual_playlist, track=track1)
        track_to_unarchive = self.model_fixture_factory.create_track_with_file(title="to unarchive")
        self.model_fixture_factory.create_track_playlist_rel(
            playlist=manual_playlist, track=track_to_unarchive)
        track3 = self.model_fixture_factory.create_track_with_file(title="not archived 3")
        self.model_fixture_factory.create_track_playlist_rel(playlist=manual_playlist, track=track3)
        track4 = self.model_fixture_factory.create_track_with_file(title="not archived 4")
        self.model_fixture_factory.create_track_playlist_rel(playlist=manual_playlist, track=track4)
        ...
        response = self._put_track(uuid=track_to_unarchive.uuid, **{PutFields.ARCHIVED: "true"})
        assert response.status_code == status.HTTP_200_OK
        ...
        track_playlist_rels_of_playlist_archived: QuerySet[TrackPlaylistRel] = \
            TrackPlaylistRel.objects.filter(user=self.test_user1, playlist=manual_playlist, position__isnull=True)
        assert track_playlist_rels_of_playlist_archived.count() == 1
        assert cast(TrackPlaylistRel, track_playlist_rels_of_playlist_archived.first()
                    ).track == track_to_unarchive

        response = self._put_track(uuid=track_to_unarchive.uuid, **{PutFields.ARCHIVED: "false"})
        ...
        assert TrackPlaylistRel.objects.filter(
