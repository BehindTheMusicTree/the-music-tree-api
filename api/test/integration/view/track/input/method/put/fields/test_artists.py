...
class TestCase(TrackTestCase, PutBodyDataTestCase):
    ...
    def test_not_provided_then_unchanged(self):
        ...
        track = self.model_fixture_factory.create_track_with_file(title="Love", artists=[artist])

        response = self._put_track(track.uuid, **{PutFields.TITLE: "Lova"})
    ...
    def test_empty_then_none(self):
        ...
        track = self.model_fixture_factory.create_track_with_file(title="koko", artists=[artist_old])

        response = self._put_track(uuid=track.uuid, **{PutFields.ARTISTS_NAMES_MULTIPART: []})
    ...
    def test_delete_old_one_because_nothing_linked_to_it(self):
        ...
        track = self.model_fixture_factory.create_track_with_file(title="Foire", artists=[artist])
    ...
    def test_not_delete_old_one_because_a_track_linked_to_it(self):
        ...
        artist = self.model_fixture_factory.create_artist(name=artist_name)
        track = self.model_fixture_factory.create_track_with_file(title="Foire", artists=[artist])
        self.model_fixture_factory.create_track_with_file(title="Josie", artists=[artist])

        response = self._put_track(uuid=track.uuid, **{PutFields.ARTISTS_NAMES_MULTIPART: [artist_name]})
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_not_delete_old_one_because_an_album_with_a_track_linked_to_it(self):
        ...
        self.model_fixture_factory.create_track_with_file(title="Josie", album=album)
