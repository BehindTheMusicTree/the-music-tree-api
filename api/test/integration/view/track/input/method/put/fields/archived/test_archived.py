...
class TestCase(TrackTestCase, PutBodyDataTestCase):
    ...
    def test_not_provided_then_unchanged(self):
        ...
        response = self._put_track(uuid=track.uuid, **{PutFields.TITLE: "Love"})

        assert response.status_code == status.HTTP_200_OK
        assert not self.saved_object.archived

        track = self.model_fixture_factory.create_track_with_file(title="Love", archived=True)
    ...
    def test_empty_then_400_bad_request(self):
        track = self.model_fixture_factory.create_track_with_file(title="Love")
        response = self._put_track(uuid=track.uuid, **{PutFields.ARCHIVED: ''})
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_not_boolean_then_400_bad_request(self):
        track = self.model_fixture_factory.create_track_with_file(title="Love")
        response = self._put_track(uuid=track.uuid, **{PutFields.ARCHIVED: 'koko'})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
