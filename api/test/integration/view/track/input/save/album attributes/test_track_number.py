...
class TestCase(TrackTestCase, NullablePositiveIntBodyDataTestCase):
    ...
    def test_empty_then_none(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: None})

        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_object.track_number == None

    def test_empty_string_then_none(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: ''})
    ...
    def test_string_not_castable_then_400_bad_request(self):
        response = self._post_track(**{PostFields.ALBUM_NAME: 'album', PostFields.TRACK_NUMBER: 'five'})
    ...
    def string_castable_then_ok(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: '5'})

        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_object.track_number == 5

    def test_zero_then_400_bad_request(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: 0})
    ...
    def test_largest_then_ok(self):
        track_number = settings.TRACK_TRACK_NUMBER_MAX
        response = self._post_track(**
                                    {PostFields.ALBUM_NAME: 'album', PostFields.TRACK_NUMBER: track_number})
    ...
    def test_too_large_then_400_bad_request(self):
        response = self._post_track(**{PostFields.ALBUM_NAME: 'album',
                                    PostFields.TRACK_NUMBER: settings.TRACK_TRACK_NUMBER_MAX + 1})
    ...
    def test_negative_then_400_bad_request(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: -1})
    ...
    def test_float_then_400_bad_request(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: 5.5})
    ...
    def test_multi_value_then_400_bad_request(self):
        response = self._post_track(**{PostFields.TRACK_NUMBER: [1, 2]})
