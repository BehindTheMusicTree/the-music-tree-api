...
class TestCase(TrackTestCase):
    ...
    def test_language_and_genre_name_then_ok(self):
        ...
        self.model_fixture_factory.create_track_with_file(title="Hey", language="fr")
        self.model_fixture_factory.create_track_with_file(title="Rockaille", language="en")

        response = self._list_tracks(language='en', genre_name='Roc')

        assert response.status_code == status.HTTP_200_OK
        assert self.results_overall_total == 1
        assert self.results[0][data_transformer.to_camel_case(TrackFields.TITLE)] == track.title
    ...
    def test_title_and_album_name_and_artists_name_ok(self):
        ...
        self.model_fixture_factory.create_track_with_file(
            title="Life", language="en", genre=genre)
        track_pascalito = self.model_fixture_factory.create_track_with_file(title="Pascalito",
                                                                            album=album_best,
                                                                            artists=[artist_john])
        track_mapasa = self.model_fixture_factory.create_track_with_file(title="mapasa",
                                                                         album=album_besto,
                                                                         artists=[artist_john, artist_jony])
        self.model_fixture_factory.create_track_with_file(title="Hey", album=album_best, artists=[artist_john])
        self.model_fixture_factory.create_track_with_file(title="sd", album=album_besto, artists=[artist_jony])
        self.model_fixture_factory.create_track_with_file(title="Hey",
                                                          album=album_best,
                                                          artists=[artist_john, artist_jony])

        response = self._list_tracks(title='pas', album_name='Best', artists_name='Joh')

        assert response.status_code == status.HTTP_200_OK
        assert self.results_overall_total == 2
        titles = self.results[0][
            data_transformer.to_camel_case(TrackFields.TITLE)], self.results[1][
            data_transformer.to_camel_case(TrackFields.TITLE)]
    ...
    def test_genre_name_and_created_on_range_then_ok(self):
        ...
        track_rock_in_range = self.model_fixture_factory.create_track_with_file(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert self.results_overall_total == 2
        titles = [result[data_transformer.to_camel_case(TrackFields.TITLE)] for result in self.results]
