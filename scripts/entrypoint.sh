...
REQUIRES_VARS=(
    INIT_IF NECESSARY_DB_AND_ROLE_SCRIPT
    APP_PORT
    GUNICORN_LOG_DIR
...
if [ -z "INIT_IF NECESSARY_DB_AND_ROLE_SCRIPT" ]; then
    echo "INIT_IF NECESSARY_DB_AND_ROLE_SCRIPT must be set." >&2
    exit 1
fi

OUTPUT=$(bash ${INIT_IF NECESSARY_DB_AND_ROLE_SCRIPT})
if [ $? -ne 0 ]; then
