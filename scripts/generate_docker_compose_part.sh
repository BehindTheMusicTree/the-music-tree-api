...
required_vars=(
  ...
  AUDIO_FINGERPRINTER_DOCKERIZED_POOL_DIR
  AUDIO_FINGERPRINTER_DOCKERIZED_FLASK_LOG_DIR
...
cat << EOF > ${SCRIPTS_DIR}$DOCKER_COMPOSE_PART_FILENAME
  ...
  audio_fingerprinter:
    ...
    volumes:
      - api-upload-temp-files:$AUDIO_FINGERPRINTER_DOCKERIZED_POOL_DIR
      - afp-flask-log-dir:$AUDIO_FINGERPRINTER_DOCKERIZED_FLASK_LOG_DIR
      - afp-gunicorn-log-dir:$AUDIO_FINGERPRINTER_DOCKERIZED_GUNICORN_LOG_DIR
    ports:
